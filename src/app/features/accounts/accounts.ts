...
import AccountAdd from './ui/account-add';
import {
  lucideChevronDown,
  lucideChevronLeft,
  lucideChevronRight,
...
import AccountDetail from './ui/account-detail';
...
export default class Accounts extends BaseEntityList<Account> {
  ...
  private readonly _accountsQuery = injectQuery(() => ({
    queryKey: ['accounts'],
    queryFn: () => this._accountsService.getAllAccounts(),
    initialData: [],
    retry: 1,
    refetchOnWindowFocus: true,
  }));

  override get data(): Account[] {
    return this._accountsQuery.data();
  }

  override columns: ColumnDef<Account>[] = [
    {
      id: 'select',
    ...
    },
    {
      ...
      cell: () =>
        flexRenderComponent(ActionDropdown, {
          outputs: {
            view: (account: Account) => this.viewAccount(account),
          },
        }),
    },
  ];

  protected viewAccount(data: Account): void {
    const context: DialogData<Account> = {
      title: 'Account Details',
      description: 'View account information and transaction history.',
      icon: 'lucideUserRound',
      data,
    };

    const dialogRef = this._hlmDialogService.open(AccountDetail, {
      context,
      contentClass: 'sm:!max-w-[750px] w-[660px]',
    });
  ...
  protected addNew(): void {
    const context: DialogData<Account | null> = {
