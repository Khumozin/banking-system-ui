...
export interface Config {
  APP_ACCOUNTS_API_URL: string;
  APP_TRANSACTIONS_API_URL: string;
}
...
export class ConfigService {
  ...
  #validateConfig(data: unknown): void {
    ...
    const requiredKeys: (keyof Config)[] = ['APP_ACCOUNTS_API_URL', 'APP_TRANSACTIONS_API_URL'];
    ...
    // Validate that APP_ACCOUNTS_API_URL is a non-empty string
    if (typeof config['APP_ACCOUNTS_API_URL'] !== 'string') {
      throw new Error('APP_ACCOUNTS_API_URL must be a string');
    }

    // Validate that APP_TRANSACTIONS_API_URL is a non-empty string
    if (typeof config['APP_TRANSACTIONS_API_URL'] !== 'string') {
      throw new Error('APP_TRANSACTIONS_API_URL must be a string');
    }
  }
